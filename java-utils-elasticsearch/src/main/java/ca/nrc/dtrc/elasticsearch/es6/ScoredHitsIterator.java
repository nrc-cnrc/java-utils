...
public class ScoredHitsIterator<T extends Document> implements Iterator<Hit<T>> {
	...
	private void retrieveAndFilterUntilNonEmptyBatch(List<Hit<T>> initialBatch) throws ElasticSearchException, SearchResultsException {
		Logger tLogger = Logger.getLogger("ca.nrc.dtrc.elasticsearch.es6.ScoredHitsIterator.retrieveAndFilterUntilNonEmptyBatch");
	...
	public boolean hasNext() {
		Logger logger = Logger.getLogger("ca.nrc.dtrc.elasticsearch.es6.ScoredHitsIterator.hasNext");
		Boolean answer = null;
	...
	@Override
	public Hit<T> next() {
		Logger logger = Logger.getLogger("ca.nrc.dtrc.elasticsearch.es6.ScoredHitsIterator.next");
		Hit<T> nextItem = null;
		if (!hasNext()) {
