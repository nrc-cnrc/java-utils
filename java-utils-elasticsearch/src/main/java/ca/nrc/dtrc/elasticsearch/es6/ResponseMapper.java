...
import static ca.nrc.dtrc.elasticsearch.es6.ErrorHandlingPolicy.STRICT;
...
public class ResponseMapper {
	...
	private void init__ResponseMapper(String _indexname,
		...
		errorLogger = Logger.getLogger("ca.nrc.dtrc.elasticsearch.es6.ResponseMapper");
	...
	public <T extends Document> T response2doc(
		...
		Logger logger = Logger.getLogger("ca.nrc.dtrc.elasticsearch.es6.ResponseMapper.response2doc");
		T doc = null;
		...
		if (logger.isTraceEnabled()) {
			if (doc != null) {
				logger.trace("Returning doc with id=" + doc.getId() + ": " + PrettyPrinter.print(doc));
			} else {
				logger.trace("Returning doc: " + PrettyPrinter.print(doc));
			}
		}
